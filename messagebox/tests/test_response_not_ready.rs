...
pub mod test {
    ...
    async fn update_identifier(id: &mut RedbIdentifier, km: &CryptoBox, witness_id: BasicPrefix) {
        ...
        id.finalize_rotate(rot_event.as_bytes(), signature)
            .await
            .unwrap();
        ...
        // Query with wrong signature
        {
            let qry = query[0].clone();
            let sig = SelfSigningPrefix::Ed25519Sha512(km.sign(&qry.encode().unwrap()).unwrap());
            id.finalize_query_mailbox(vec![(qry, sig)]).await.unwrap();
        }
    }
    ...
    async fn test_response_not_ready() {
        ...
        let witness_id: BasicPrefix = "BJq7UABlttINuWJh1Xl2lkqZG4NTdUdqnbFJDa6ZyxCC"
            .parse()
            .unwrap();
        let mut querying_identifier = setup_identifier(&km1, quering_id_db.path()).await;
        ...
        let msg_box = MessageBox::setup(db, messagebox_db.path(), messagebox_oobi_db.path(), watcher_oobi, "http://url.com".parse().unwrap(), None, Some("AAAAky1v068:APA91bHHpGtP6M5h3ICFc9AzY35MrkTmjwblkLlEJ1C0yvkrUu7KDkmkXMzPq2q-0o1l49fKxOeDQaKIkZTTEAIX3Jd45j6KNtSempYqop4Psitvz2Ng7iBz-IeS1SGEs1GpnWseJlpP".to_string()), None, None, None, None).await.unwrap();
        ...
        km1.rotate().unwrap();
        update_identifier(&mut querying_identifier, &km1, witness_id).await;
        ...
        let signed_exn = inserting_identifier
            .sign_with_index_to_cesr(&exn_msg.to_string(), signature, 0)
            .unwrap();
        ...
        let signed_query = querying_identifier
            .sign_with_index_to_cesr(&qry_msg.to_string(), signature, 0)
            .unwrap();
